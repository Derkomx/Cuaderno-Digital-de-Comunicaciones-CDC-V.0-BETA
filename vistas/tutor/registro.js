...
function Registro() {
    ...
    if (dni.length == 0) {
        ...
        setTimeout(() => { document.getElementById("dni").style.borderColor = "LightGray"; }, 3000);
    }

    if (nombre.length == 0) {
        Incompleto = true;
        document.getElementById("nombre").style.borderColor = "LightCoral";
        setTimeout(() => { document.getElementById("nombre").style.borderColor = "LightGray"; }, 3000);
    }

    if (apellido.length == 0) {
        Incompleto = true;
        document.getElementById("apellido").style.borderColor = "LightCoral";
        setTimeout(() => { document.getElementById("apellido").style.borderColor = "LightGray"; }, 3000);
    }

    if (contraseña.length == 0) {
        Incompleto = true;
        document.getElementById("contraseña").style.borderColor = "LightCoral";
        setTimeout(() => { document.getElementById("contraseña").style.borderColor = "LightGray"; }, 3000);
    }

    if (Ccontraseña.length == 0) {
        Incompleto = true;
        document.getElementById("Ccontraseña").style.borderColor = "LightCoral";
        setTimeout(() => { document.getElementById("Ccontraseña").style.borderColor = "LightGray"; }, 3000);
    }
    ...
    $.ajax({
        ...
        url: 'Inyector.php',
        data: { 
            Archivo: 'Registrotutor.php', 
            dni: dni, 
            contraseña: contraseña, 
            nombre: nombre, 
            apellido: apellido 
        },
        dataType: 'json', // Cambié a 'json' para manejar mejor la respuesta
...
document.getElementById('contraseña').addEventListener('input', function(event) {
    ...
    if (Valor.length > 0) {
        // Muestra los elementos si están ocultos
        Barra.style.display = 'block';
        Texto.style.display = 'block';
    } else {
        // Oculta los elementos si están visibles
